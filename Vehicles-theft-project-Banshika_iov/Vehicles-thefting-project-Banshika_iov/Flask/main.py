...
def sends_otp():
    if request.method == 'POST':
        ...
        if 'vname' in session:
                Credentials = project.find({'VIN':session['vname']},{'_id':0})
                for cred in Credentials:   
                    if method_ == 'email':
                        cred_mail = cred['Mail ID']
                        if cred_mail==email_phone:
                            otp_mail(email_phone,OTP_str)
                            session['otp_number'] = OTP_str
                            flash('OTP sent successfully!', 'success')
                            return render_template('otp_verify.html')
                        else:
                            flash('Email is not registered with the vehicle','danger')
                            render_template('sends_otp.html')
                    elif method_ == 'phone':
                        cred_phone = cred['Phone no']
                        if cred_phone==email_phone:
                            otp_phone(email_phone,OTP_str)
                            session['otp_number'] = OTP_str
                            flash('OTP sent successfully!', 'success')
                            return render_template('otp_verify.html')
                        else:
                            flash('Phone number is not registered with the vehicle','danger')
                            render_template('sends_otp.html')
